...
export default function HomePage() {
  ...
  const [activeTab, setActiveTab] = useState<'外来' | '入院'>('外来');
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b-2 border-gray-300 shadow-sm">
        <div className="flex">
          <button
            onClick={() => setActiveTab('外来')}
            className={`px-4 md:px-6 py-2 md:py-2.5 text-xs md:text-sm border-r border-gray-300 transition-colors ${
              activeTab === '外来'
          ...
          <button
            onClick={() => setActiveTab('入院')}
            className={`px-4 md:px-6 py-2 md:py-2.5 text-xs md:text-sm transition-colors ${
              activeTab === '入院'
                ? 'bg-blue-50 text-blue-700 font-semibold border-b-2 border-blue-600'
                : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'
            }`}
      ...
      {(() => {
        const filteredPatients = patients.filter(p =>
          activeTab === '入院'
            ? !!p.admissionDate && !p.dischargeDate
            : !p.admissionDate || !!p.dischargeDate
        );
        return (
            ...
            <table className="w-full border-collapse text-sm">
              <thead className="bg-gradient-to-b from-gray-200 to-gray-250 border-b-2 border-gray-400">
                <tr>
                  ...
                  {activeTab === '入院' ? (
                    ...
                    <>
                      <th className="border border-gray-400 px-3 py-2.5 text-left w-28 text-gray-700 font-semibold">最終受診日</th>
                      <th className="border border-gray-400 px-3 py-2.5 text-left w-28 text-gray-700 font-semibold">担当医</th>
                      <th className="border border-gray-400 px-3 py-2.5 text-left w-40 text-gray-700 font-semibold">主訴</th>
                    </>
                  )}
                </tr>
              </thead>
              <tbody>
                {filteredPatients.map((patient, index) => {
                  const lastVisit = patient.visits?.sort((a, b) => b.date.localeCompare(a.date))[0];
                  return (
                    <tr
                      key={patient.id}
                    ...
                    >
                      ...
                      {activeTab === '入院' ? (
                        <>
                          ...
                          <td className="border border-gray-300 px-3 py-2.5 bg-white text-xs text-gray-700">{patient.admissionDiagnosis || '-'}</td>
                          <td className="border border-gray-300 px-3 py-2.5 bg-white text-xs text-gray-700">{patient.dpcDiagnosis || '-'}</td>
                          <td className="border border-gray-300 px-3 py-2.5 bg-white text-gray-700">{patient.dpcPeriod || '-'}</td>
                        </>
                      ) : (
                        <>
                          <td className="border border-gray-300 px-3 py-2.5 bg-white text-gray-700">{lastVisit?.date || '-'}</td>
                          <td className="border border-gray-300 px-3 py-2.5 bg-white text-gray-700">{patient.outpatientAttendingPhysician || '-'}</td>
                          <td className="border border-gray-300 px-3 py-2.5 bg-white text-xs text-gray-700">{patient.chiefComplaint || '-'}</td>
                        </>
                      )}
                    </tr>
                  );
